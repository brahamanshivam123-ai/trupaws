...
export default function SignupModal({ intent, onClose, onSuccess }) {
  ...
  const handleSubmit = (e) => {
    ...
    setLoading(true);
    // Simulate brief async signup
    setTimeout(() => {
      onSuccess({ ...form, role, id: Date.now().toString() });
    }, 900);
  };
              ...
              >
                ...
                {LOCATIONS.map((loc) => (
                  <option
                    key={loc}
                    value={loc}
                    style={{ background: '#0E2412', color: '#F5F0E8' }}
                  >
                    {loc}
                  </option>
                ))}
